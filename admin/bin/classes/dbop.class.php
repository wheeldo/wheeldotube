...
class dbop {
	...
	public function __construct() {
		$this->conn = $sqldb = mysql_connect( DB_HOST, USER, PASSWORD ) or die( "error - unable to connect to database server" );
	...
	public function selectDB( $tname, $wherecase = "" , $show_sql=false) {
		/*
	...
	public function selectAssocRow( $tname, $wherecase = "" ,$show_sql=false) {

		$tname = $this->cleanQuery( $tname );
		$wherecase = $this->cleanBadWords( $wherecase );
		$sql = "SELECT * FROM `$tname` $wherecase";
                if($show_sql) echo "<pre>". $sql ."</pre>";
		$result = mysql_query( $sql );
		if ( $result )
