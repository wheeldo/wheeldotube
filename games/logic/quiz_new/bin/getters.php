...
class getters extends dbop{
    ...
    public function getStartPage() {
        ...
        $game=$this->selectAssocRow("games","WHERE `game_id`='{$game_id}'");
        if($game&&($game['instruction_text']!=""||$game['instruction_img']!="")) {
            $res['status']="ok";
            $res['game']=$game;
        }
        else {
    ...
    public function loadGameEnd() {
        ...
        $only_reg=false;
        $res['leaders']=array();
        $ans=$this->selectDB("game_quiz_user","WHERE `game_id`='{$game_id}' AND `ghost`=0  ORDER BY `score` DESC LIMIT 0,3");
        if($ans['n']<3) {
            $only_reg=true;
