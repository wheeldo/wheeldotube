...
//        });
    ...
    $scope.inst_init = function() {   
      ...
      if(referrer) {
        ...
        $scope.no_inst_class="";
        $http.post(BASE+'/gt',{op:"getStartPage"})
        .success(function(data, status, headers, config) {
           if(data.status=="ok") {
               $scope.no_inst_class="no_inst_class";
               $scope.startPage=data.game;
           }
    ...
    setScoreBar = function() {
        ...
        var per=Math.ceil(($scope.game_progress.answerd-1)/$scope.game_progress.total*100);
...
//        }
        ...
        if($scope.$$phase || $scope.$root.$$phase) {
                ...
                if($scope.game_progress.answerd<$scope.game_progress.total) {
    ...
    getEndData = function() {
        ...
        .success(function(data, status, headers, config) {
            if(data.total=="0") {
                 setTimeout(getEndData,600);
                 return;
            }
            $scope.end_data=data;

            $scope.result_text='I answered '+data.right+' out of '+data.total+' questions correctly and I won '+data.score+' points at ';
    ...
    get5050Service = function(q_id) {
        $http.post(BASE+'/gt',{op:"get5050",q_id:q_id})
            .success(function(data, status, headers, config) {
                if(data.status=="ok") {
                    for(i in data.dis){
                        var d=data.dis[i];
