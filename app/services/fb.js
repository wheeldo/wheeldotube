function appCtrl(Facebook, $scope, $rootScope, $http, $location) {
    ...
    $rootScope.$on("fb_connected", function (event, args) {
        ...
        function authenticateViaFacebook(parameters) {
            $http.post('/op', {op:'isFacebookUser', facebook_uid:parameters.facebook_id}).success(function(data, status, headers, config){
            	if(data.is_user) {
            		// already registered as a Facebook user, now log him in
            		$http.post('/op', {op:'loginFacebookUser',token:FB.getAccessToken()}).success(function(data, status, headers, config){
            			stopProcessing();
                                
                                 

            			if(data.status == 'ok') {
                                    if($scope.FbonloginFunc) {
                                        $scope.FbonloginFunc();
                                    }
                                    else {
            		...
            		$http.post('/op', {op:'registerFacebookUser',token:FB.getAccessToken()}).success(function(data, status, headers, config){
            			stopProcessing();
    ...
    $scope.loginNoReload = function (hitt_func) {
        $scope.FbonloginFunc=hitt_func;
    	setProcessing();
        Facebook.login();
    };
